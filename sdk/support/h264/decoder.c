...
void decoder_free(decoder* d) {
  ...
  if (d->frame_rgb) av_frame_free(&(d->frame_rgb));

  if (d->codec_context) avcodec_free_context(&(d->codec_context));
...
int decoder_parse_data(decoder* d, const char* data, int size,
  ...
  return av_parser_parse2(d->codec_parser_context, d->codec_context,
                          (uint8_t**)&parsed_data, parsed_data_size,
...
void decoder_send_data(decoder* d, char* data, int size) {
  ...
  while (size > 0) {
    int len = av_parser_parse2(d->codec_parser_context, d->codec_context,
                               &parsed_data, &parsed_data_size,
    ...
    if (parsed_data_size > 0) {
      ...
      if (avcodec_receive_frame(d->codec_context, d->frame) != 0) continue;

      const AVFrame* frame = d->frame;

      d->sws_context =
          sws_getCachedContext(d->sws_context, frame->width, frame->height,
      ...
      av_image_fill_arrays(d->frame_rgb->data, d->frame_rgb->linesize,
                           d->output_buffer, AV_PIX_FMT_RGB32, frame->width,
                           frame->height, 1);
      ...
      d->frame_callback(d->output_buffer, d->output_buffer_size, d->user_data);
    }
  }
}
