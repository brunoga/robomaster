...
typedef void(*EventCallbackFunc)(uint64_t event_code, uint8_t* data,
		int length, uint64_t tag);

// CreateUnityBridge creates a new bridge with the given name and debuggable
// status. Log will be written to the file represented by log path. This
// should always be called before anything else and only once.
void CreateUnityBridge(const char* name, bool debuggable,
		const char* log_path);
...
void UnitySendEvent(uint64_t event_code, uint8_t* data, uint64_t tag);
